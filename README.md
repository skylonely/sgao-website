...
主站首页的「我的工作台」集中展示 Todo、旅行、知识库和图片管理入口。Todo 卡片提供“新建”快捷操作，通过一次性 `?create=1` 深链打开 Todo 原有的新建清单弹窗，Todo 加载后会移除该参数，刷新不会重复打开；图片卡片提供“上传图片”快捷入口。登录后会从 Todo 账号接口只读获取当前清单顺序、待完成数量及前 3 张清单；未登录时不请求或展示私人清单，读取失败时仅显示重试提示。

工作台底部的「统一备份中心」集中提供导航数据下载与恢复、Todo 备份与恢复、图片 ZIP 导出与恢复入口。Todo 使用一次性 `?backup=1` 深链打开原有备份弹窗，图片恢复入口使用 `#backup` 定位到原有恢复面板；三个系统仍使用各自独立的备份格式并下载到本机，不新增云端备份，也不能交叉导入。验证：`npm run workbench:test`、`npm run todo:test`。
...
删除自定义网站或分类后，底部提示可撤销最近一次删除，不自动超时。再次成功删除、关闭提示、成功导入/恢复备份、清空本机数据或刷新页面会替换或清除撤销入口；切换账号时不会显示其他账号的撤销操作。撤销记录只保留在当前页面内存，不上传或写入备份格式，不是完整回收站。

网站撤销恢复同一 ID、原分类、收藏与访问足迹，并按仍存在的原邻居恢复位置；其他条目的新编辑、添加和排序不重置。分类删除仍把网站移至实用工具，撤销恢复分类及原所属网站的归属，不重置网站最新顺序。相关网站已被修改/删除、原分类不存在、名称/ID 冲突或恢复后数量超限时拒绝撤销，不覆盖最新数据或自动复活其他条目。确认删除前读取最新数据，并拒绝删除已被修改的条目。

删除与撤销复用现有安全保存流程，每次操作前保存最近一份本机自动备份；刷新后可到“偏好与数据 → 最近一次操作前备份”预览恢复，但会恢复整份快照，请先检查预览。全部字段保存成功才通知账号同步；失败尝试回滚，回滚失败暂停同步并保留可下载备份。同步仍只上传收藏、自定义网站和分类，足迹和撤销记录只在本机保留。验证：`npm run delete:test`、`npm run navigation:test`。
