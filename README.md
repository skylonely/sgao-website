...
主站首页的「我的工作台」集中展示 Todo、旅行、知识库和图片上传入口。Todo 卡片提供“新建”快捷操作，通过一次性 `?create=1` 深链打开 Todo 原有的新建清单弹窗，Todo 加载后会移除该参数，刷新不会重复打开。登录后会从 Todo 账号接口只读获取当前清单顺序、待完成数量及前 3 张清单；未登录时不请求或展示私人清单，读取失败时仅显示重试提示。工作台不新增云端存储或清单写入。验证：`npm run workbench:test`、`npm run todo:test`。
